...
// ── Phase 2: Data check (progressive IEND search) ──
// Walks chunk structure looking for IEND chunk.
ValidateResult check_png_data_impl(const uint8_t* data, size_t length, uint64_t offset_in_file, uint64_t& calculated_file_size) {
    // We need to walk the chunk stream. The scanner provides us with a block
    // starting at offset_in_file. For simplicity, scan for IEND signature pattern
    // (4-byte length of 0 + "IEND" type + 4-byte CRC).
    // This is less precise than full chunk walking but sufficient for progressive search.

    // Look for the IEND chunk pattern: 00 00 00 00 49 45 4E 44 AE 42 60 82
