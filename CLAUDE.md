...
**禁止从 git 回滚代码。** 不得使用 `git reset`、`git revert`、`git checkout` 等命令回滚已提交的代码。如需修正错误，应通过新增提交或编辑当前工作区解决。

## 开发规范

**开发完成后自动提交。** 功能开发完成且编译通过后，自动执行 `git add` 和 `git commit` 提交代码，无需等待用户确认。
